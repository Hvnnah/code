...
function makeFakeZtkBinary(): string {
  const dir = mkdtempSync(join(tmpdir(), "ztk-test-"));
  const binaryPath = join(dir, "ztk");
...
describe("ztk add-on", () => {
  ...
  beforeEach(() => {
    binaryPath = makeFakeZtkBinary();
  });

  it("rejects unknown options keys", () => {
    expect(() => ztkAddOn.parseOptions({ binaryPath: 42 })).toThrow();
  });

  it("throws from prepare() when the configured binary is missing", () => {
    expect(() =>
      ztkAddOn.prepare?.(
        makeCtx(),
        ztkAddOn.parseOptions({ binaryPath: "/does/not/exist/ztk" }),
      ),
    ).toThrow(/binary not found/);
  });

  it("contributes a PreToolUse hook that rewrites Bash commands", async () => {
    const contribution = await ztkAddOn.contribute(
      makeCtx(),
      ztkAddOn.parseOptions({ binaryPath }),
    );
  ...
  it("appends --skip-permissions when configured", async () => {
    const contribution = await ztkAddOn.contribute(
      makeCtx(),
      ztkAddOn.parseOptions({ binaryPath, skipPermissions: true }),
    );
    const hook = firstHook(contribution);
    const result = (await runHook(hook, {
      tool_name: "Bash",
  ...
  it("escapes single quotes inside the binary path", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "ztk-quote-"));
    const quoted = join(dir, "weird'name");
    mkdirSync(quoted, { recursive: true });
    const bp = join(quoted, "ztk");
    writeFileSync(bp, "#!/bin/sh\nexit 0\n", { mode: 0o755 });

    const contribution = await ztkAddOn.contribute(
      makeCtx(),
      ztkAddOn.parseOptions({ binaryPath: bp }),
    );
    const hook = firstHook(contribution);
    const result = (await runHook(hook, {
      tool_name: "Bash",
